...
public class CurveMeshBuilder : MonoBehaviour
{
    ...
    private List<Vector2> GetVertices(List<Vector2> points, float expands)
    {
        ...
        for (int i = 0; i < segments1.Count; i++)
        {
            ...
            {
                Vector2 crossPoint;
                if (!TryCalculateLinesIntersection(segments1[i - 1], segments1[i], out crossPoint))
                {
        ...
        for (int i = 0; i < segments2.Count; i++)
        {
            ...
            {
                Vector2 crossPoint;
                if (!TryCalculateLinesIntersection(segments2[i - 1], segments2[i], out crossPoint))
                {
    ...
    private bool TryCalculateLinesIntersection(CurveSegment2D segment1, CurveSegment2D segment2, out Vector2 intersection)
    {
